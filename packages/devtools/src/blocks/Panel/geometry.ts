...
import type { WindowEdgeName } from '../../uikit/Window/index.js'
...
// How the width and the height follow the pointer by each edge: along it, against it, or not at all
const EDGE_SIGNS: Record<WindowEdgeName, [x: number, y: number]> = {
  left: [-1, 0],
  right: [1, 0],
  bottom: [0, 1],
  bottomLeft: [-1, 1],
...
/**
 * The size a window gets when an edge or a lower corner of it is dragged: the dragged edges follow the pointer,
 * the opposite ones stay, and the size keeps between the least one and the edges of the viewport.
 * @param from - The frame the drag began with.
 * @param dx - How far the pointer went to the right.
 * @param dy - How far the pointer went down.
 * @param viewport - The viewport.
 * @param edge - The edge or the corner that is dragged.
 * @returns The frame.
 */
export function resizeFrame(
  ...
  const [x, y] = EDGE_SIGNS[edge]
  const left = viewport.width - from.right - from.width
  const top = viewport.height - from.bottom - from.height
  const width = x
    ? clamp(from.width + x * dx, MIN_WIDTH, viewport.width - (x > 0 ? left : from.right))
    : from.width
  const height = y
    ? clamp(from.height + dy, MIN_HEIGHT, viewport.height - top)
    : from.height

  return {
    right: x > 0 ? viewport.width - left - width : from.right,
    bottom: y ? viewport.height - top - height : from.bottom,
    width,
    height
  }
}
