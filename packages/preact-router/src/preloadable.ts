...
function createUsePreloadHook(
  ...
  return function usePreload(
    ...
    const onFocusCallback = useEventCallback((event: TargetedFocusEvent<HTMLAnchorElement>) => {
      onFocus?.(event)
      preloadCallback()
    })
    const onMouseEnterCallback = useEventCallback((event: TargetedMouseEvent<HTMLAnchorElement>) => {
      onMouseEnter?.(event)
      preloadCallback()
    })
