...
`@nano_kit/platform-web` provides small reactive wrappers around browser APIs. The package is built on top of [`@nano_kit/store`](/store), so every helper returns a signal that can be used with [`effect`](/store/core-concepts/#signals), [`computed`](/store/core-concepts/#using-computed), framework integrations, query settings, or your own store utilities.
...
$theme('dark')
$theme(null) /* Calls cookieStore.delete('theme') */
```
...
Assigning `null` or `undefined` deletes the underlying cookie through the provided [`CookieStore`](https://developer.mozilla.org/en-US/docs/Web/API/Cookie_Store_API)-compatible object. If a default value was provided, the signal falls back to it after deletion.
