...
`NextNavigationProvider` is the client-side provider used by the Pages Router and by `NextNavigation` after hydration. It creates an `Injector` with `LocationNavigation$` only when a fresh client navigation context is needed.
...
```tsx
import type { GetServerSideProps } from 'next'
import { Injector, dehydrate, provide } from '@nano_kit/store'
import {
  LocationNavigation$,
  notFound,
  redirect,
...
export const getServerSideProps: GetServerSideProps = async (nextContext) => {
  ...
  const dehydrated = await dehydrate(Stores$, injector)
  const { $user } = injector.get(User$)

  return notFound($user()) ?? redirect(injector) ?? {
    props: {
      dehydrated
    }
  }
