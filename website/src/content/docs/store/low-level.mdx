...
**`stored`** creates a writable signal backed by a storage adapter. The adapter only needs synchronous `get` and `set` methods, so the same helper can wrap `localStorage`, cookies, in-memory maps, or any other synchronous key/value storage.
...
const $cart = stored(storage, 'cart', [] as number[], JsonCodec, debounce(300))
