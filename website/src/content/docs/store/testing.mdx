...
Use the DI mechanism to mock dependencies in tests. Create a custom `Injector` with `provide` to inject test doubles.

```ts
import { Injector, provide, inject } from '@nano_kit/store'
...
/* Inject store with mocked injector */
const { $episodes } = inject(Episodes$, injector)
```
