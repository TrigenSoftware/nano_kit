...
Use invocable tokens with `inject` to retrieve dependencies. An invocable token can be a function or an injectable class. For example, in React, use `InjectorProvider` to create an injector and `useInject` to access dependencies.
...
/* Invocable token that defines a user store */
function User$() {
...
/* Component that provides the injector */
function App() {
  return (
    <InjectorProvider>
      <UserProfile />
...
Use `Injectable` when a dependency is naturally represented as a class. Class fields can call `inject(...)`, so services can compose other dependencies in the same injector.
...
function App() {
  return (
    <InjectorProvider injector={[provide(Theme$, 'dark')]}>
      <TopBar />
    </InjectorProvider>
  )
}
...
If an invocable token has no meaningful default and must always be provided via `provide`, throw `DependencyNotFound` to make the error explicit:

```tsx
import { DependencyNotFound, inject, provide } from '@nano_kit/store'
import { InjectorProvider, useInject } from '@nano_kit/react'
...
Invocable tokens that start with an uppercase letter and end with `$` are Dependency Injection tokens. Function tokens work well for stores, and class tokens can extend `Injectable`.
