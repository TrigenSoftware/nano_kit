...
**When to use:** Keep data up to date when the user returns to your application.
...
**When to use:** Recover data synchronization automatically after an outage, especially on mobile devices or unreliable networks.
...
**When to use:** Data that changes frequently on the server, such as stock prices, live scores, or status dashboards.
...
Creates an IndexedDB storage adapter for `persistence()`, allowing cached data to survive page reloads and act as a cache for offline mode.
