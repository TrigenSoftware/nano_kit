...
<Aside>
  For app code, prefer the matching [framework integration](/integrations) instead of wiring the renderer manually.
</Aside>
...
For pages that use DI, `Stores$` is called within the request's `Injector`, so you can use `inject()` inside it:
...
`Head$` is also called within the request's `Injector`, so you can use `inject()` inside it:
